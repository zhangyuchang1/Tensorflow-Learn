...
plt.imshow(cat)
cat = tf.cast(cat, tf.float32) #[173, 237, 3]
...
with tf.variable_scope('conv1'):
    ...
    x_pool = tools.pool('test1', x_relu,
                        kernel=[1, 2, 2, 1],
                        stride=[1, 2, 2, 1],
                        is_max_pool=True)
...
with tf.variable_scope('conv2') as scope:
    ...
    x_pool2 = tools.pool('test2', x_relu2, kernel=[1, 2, 2, 1], is_max_pool=True)
